...
### JOIN ~ USING() 조인
- 두 테이블에 같은 이름을 가진 컬럼이 여러개 있을 경우, 조인할 때 사용할 특정 컬럼을 지정하는 문법이다.

예) 강의실 정보(강의번호, 강의명, 강의실명)를 출력하라
~~~~
--신규 문법
--lect와 croom 테이블에 이름이 같은 커럶이 여러 개 있을지라도, 오직 crmno 컬럼의 값이 같은 데이터끼리만 연결한다.
select lno, titl, name
...
예) 수강신청 정보(수강신청번호, 강의명, 강의실이름, 매니저명, 매니저직책, 수강생이름, 수강생이메일, 재직여부)를 조회한다.
~~~~
select lano, lno, sno
from lect_appy;
...
select lano, titl, sdt, edt, croom.name, memb.name
from lect_appy 
     left outer join lect on lect_appy.lno=lect.lno
     left outer join memb on lect_appy.sno=memb.mno
     left outer join croom on lect.crmno=croom.crmno;

select lano, titl, sdt, edt, croom.name, m2.name, memb.name
from lect_appy 
     left outer join lect on lect_appy.lno=lect.lno
     left outer join memb on lect_appy.sno=memb.mno
     left outer join croom on lect.crmno=croom.crmno
...
예) 강의실이 배정되지 않은 강의에 대해서만 수강신청번호와 강의번호, 학생번호를 출력하라.
~~~~
